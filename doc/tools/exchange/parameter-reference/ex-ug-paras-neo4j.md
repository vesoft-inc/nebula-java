...
| `tags.exec` | 无 | `string` | 是 | 从 Neo4j 数据库中检索打了某种标签的点的属性，并指定别名。<br />例如，写入 Cypher 查询语句 `match (n:label) return n.neo4j-field-0 as neo4j-field-0, n.neo4j-field-1 as neo4j-field-1 order by (n.neo4j-field-0)` |
...
| `edges.exec` | 无 | `string` | 是 | 表示从 Neo4j 数据库中查询关系属性。<br />例如，写入 Cypher 查询语句 `match (a:vertex_label)-[r:edge_label]->(b:vertex_label) return a.neo4j-source-field, b.neo4j-target-field, r.neo4j-field-0 as neo4j-field-0, r.neo4j-field-1 as neo4j-field-1 order by id(r)`  |
