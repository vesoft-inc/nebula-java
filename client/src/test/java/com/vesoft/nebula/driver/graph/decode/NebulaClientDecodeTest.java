...
public class NebulaClientDecodeTest {
    ...
    @Test
    public void testDecodeListGeoResult() {
        System.out.println("<==== testDecodeListGeoResult ====>");
        try {
            String             gql    = "RETURN list[ST_GeogFromText(\"POINT(1 2)\")]";
            ResultSet          res    = client.execute(gql);
            List<ValueWrapper> values = res.next().values();
            Assert.assertEquals(1, values.size());
            ValueWrapper value = values.get(0).asList().get(0);
            Assert.assertTrue("POINT(1.0 2.0)".equals(value.asGeography().toString()));
        } catch (Exception e) {
            e.printStackTrace();
            Assert.fail(e.getMessage());
        }
