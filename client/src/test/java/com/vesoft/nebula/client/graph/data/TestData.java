...
public class TestData {
    ...
    public void testRelationShip() {
        try {
            ...
            assert Objects.equals(
                    expectVals, longVals.stream().sorted().collect(Collectors.toList()));
    ...
    public void testResult() {
        try {
            ...
            Set<Long> set =
                    record.get(7).asSet().stream()
                            .map(ValueWrapper::asLong)
            ...
            Assert.assertArrayEquals(
                    map.keySet().toArray(), Arrays.asList("key1", "key2").toArray());
            Assert.assertArrayEquals(
                    map.values().stream().map(ValueWrapper::asLong).toArray(),
                    Arrays.asList((long) 1, (long) 2).toArray());
            ...
            TimeWrapper timeWrapper =
                    (TimeWrapper)
                            new TimeWrapper(new Time((byte) 10, (byte) 30, (byte) 0, 100))
                                    .setTimezoneOffset(28800);
            assert Objects.equals(record.get(9).asTime(), timeWrapper);
            Assert.assertEquals(
                    "utc time: 10:30:00.000100, timezoneOffset: 28800", timeWrapper.toString());
            ...
            Assert.assertEquals(
                    new Time((byte) 18, (byte) 30, (byte) 0, 100), timeWrapper.getLocalTime());
            ...
            Assert.assertEquals(
                    "utc datetime: 2020-10-10T10:30:00.000100, timezoneOffset: 28800",
                    dateTimeWrapper.toString());
            Assert.assertEquals(
                    "2020-10-10T18:30:00.000100", dateTimeWrapper.getLocalDateTimeStr());
            Assert.assertEquals("2020-10-10T10:30:00.000100", dateTimeWrapper.getUTCDateTimeStr());
            Assert.assertEquals(
                    new DateTime(
                            (short) 2020,
                            (byte) 10,
            ...
            assert Objects.equals(record.get(12).asNode(), new Node(getVertex("Tom")));

            assert record.get(13).isEdge();
            assert Objects.equals(
                    record.get(13).asRelationship(), new Relationship(getEdge("Tom", "Lily")));

            assert record.get(14).isPath();
            assert Objects.equals(record.get(14).asPath(), new PathWrapper(getPath("Tom", 3)));
            assert resultSet.toString().length() > 100;

            assert record.get(15).isGeography();
            assert Objects.equals(
                    record.get(15).asGeography().toString(),
                    new PointWrapper(new Point(new Coordinate(1.0, 2.0))).toString());

            assert record.get(16).isGeography();
            assert Objects.equals(
                    record.get(16).asGeography().toString(),
                    new PolygonWrapper(
                                    new Polygon(
                                            Arrays.asList(
                                                    Arrays.asList(
            ...
            assert Objects.equals(
                    record.get(17).asGeography().toString(),
                    new LineStringWrapper(
                                    new LineString(
                                            Arrays.asList(
                                                    new Coordinate(1.0, 2.0),
                                                    new Coordinate(2.0, 4.0))))
            ...
            assert Objects.equals(
                    record.get(18).asDuration(), new DurationWrapper(new Duration(100, 20, 1)));
        } catch (Exception e) {
            e.printStackTrace();
            assert (false);
        }
    ...
    public void testToString() {
        try {
            ...
            ValueWrapper valueWrapper =
                    new ValueWrapper(new Value(Value.VVAL, getSimpleVertex()), "utf-8", 28800);
            String expectString = "(\"vertex\" :tag1 {tag1_prop: 100} :tag2 {tag2_prop: 200})";
            Assert.assertEquals(expectString, valueWrapper.asNode().toString());

            // test relationship
            valueWrapper =
                    new ValueWrapper(new Value(Value.EVAL, getSimpleEdge(false)), "utf-8", 28800);
            expectString = "(\"Tom\")-[:classmate@10{edge_prop: 100}]->(\"Lily\")";
            Assert.assertEquals(expectString, valueWrapper.asRelationship().toString());

            valueWrapper =
                    new ValueWrapper(new Value(Value.EVAL, getSimpleEdge(true)), "utf-8", 28800);
            expectString = "(\"Lily\")-[:classmate@10{edge_prop: 100}]->(\"Tom\")";
            Assert.assertEquals(expectString, valueWrapper.asRelationship().toString());

            // test path
            valueWrapper =
                    new ValueWrapper(new Value(Value.PVAL, getSimplePath(true)), "utf-8", 28800);
            expectString =
            ...
            valueWrapper =
                    new ValueWrapper(new Value(Value.PVAL, getSimplePath(false)), "utf-8", 28800);
            expectString =
                    "(\"vertex0\" :tag0 {tag0_prop: 100})"
                            + "-[:classmate@100{edge1_prop: 100}]->"
                            + "(\"vertex1\" :tag1 {tag1_prop: 200})-[:classmate@10{edge2_prop: 200}]->"
            ...
            valueWrapper =
                    new ValueWrapper(
                            new Value(
                                    Value.GGVAL,
                                    new Geography(
                                            Geography.PTVAL, new Point(new Coordinate(1.0, 2.0)))),
                            "utf-8",
                            28800);
            ...
            valueWrapper =
                    new ValueWrapper(
                            new Value(Value.DUVAL, new Duration(100, 20, 1)), "utf-8", 28800);
            ...
            Assert.assertEquals(
                    expectDurationString, valueWrapper.asDuration().getDurationString());
