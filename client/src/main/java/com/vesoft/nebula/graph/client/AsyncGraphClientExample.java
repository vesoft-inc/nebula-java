    ...
    public static void main(String[] args) {
        ...
        try {
            try (AsyncGraphClient client = new AsyncGraphClientImpl(args[0],
                ...
                Futures.addCallback(switchSpaceCodeFuture,
                        new FutureCallback<Optional<Integer>>() {
                    @Override
                    public void onSuccess(@Nullable Optional<Integer> integerOptional) {
                        if (integerOptional.isPresent()) {
                            ...
                            if (code != ErrorCode.SUCCEEDED) {
                                ...
                                LOGGER.error(String.format("Please confirm %s have been created",
                                        SPACE_NAME));
                                System.exit(-1);
                            } else {
                ...
                try {
                    ...
                    Futures.addCallback(executeQueryCodeFuture,
                            executeQueryCallback, EXECUTOR_SERVICE);
