...
/**
 * The Session is an object that operates with nebula-graph. It provides an interface `execute` to
 * send any NGQL. The returned data result `ResultSet` include wrapped string encoding and time zone
 * calculations and Node and Relationship and PathWrapper and DateWrapper and TimeWrapper and
 * DateTimeWrapper. The data type obtained by the user is `ValueWrapper`, which is the wrapper of
 * the original data structure Value returned by the server. The user can directly read the data
...
public class Session implements Serializable {
    ...
    public synchronized ResultSet execute(String stmt) throws IOErrorException {
        return executeWithParameter(stmt, (Map<String, Object>) Collections.EMPTY_MAP);
    }
    ...
    public synchronized ResultSet executeWithParameter(
            String stmt, Map<String, Object> parameterMap) throws IOErrorException {
        if (connection == null) {
            throw new IOErrorException(
                    IOErrorException.E_CONNECT_BROKEN,
        ...
        if (connectionIsBroken.get() && retryConnect) {
            if (retryConnect()) {
                ...
                throw new IOErrorException(
                        IOErrorException.E_ALL_BROKEN, "All servers are broken.");
            }
        }
    ...
    /**
     * Execute the nGql sentence. Date and Datetime will be returned in UTC JSON struct: {
     * "results":[ { "columns":[], "data":[ { "row":row-data, "meta":metadata } ], "latencyInUs":0,
     * "spaceName":"", "planDesc ":{ "planNodeDescs":[ { "name":"", "id":0, "outputVar":"",
     * "description":{ "key":"" }, "profiles":[ { "rows":1, "execDurationInUs":0,
     * "totalDurationInUs":0, "otherStats":{} } ], "branchInfo":{ "isDoBranch":false,
     * "conditionNodeId":-1 }, "dependencies":[] } ], "nodeIndexMap":{}, "format":"",
     * "optimize_time_in_us":0 }, "comment ":"", } ], "errors":[ { "code": 0, "message": "" } ] }
     *
     * @param stmt The nGql sentence. such as insert ngql `INSERT VERTEX person(name) VALUES
     *     "Tom":("Tom");` parameterMap The nGql parameters
     ...
     * @return The JSON string
     */
    public synchronized String executeJsonWithParameter(
            String stmt, Map<String, Object> parameterMap) throws IOErrorException {
        if (connection == null) {
            throw new IOErrorException(
                    IOErrorException.E_CONNECT_BROKEN,
    ...
    /**
     * Notifies the server that the session is no longer needed and returns the connection to the
     * pool, and the connection will be reuse. This function is called if the user is no longer
     * using the session.
     */
    ...
    /**
     * set current connection is invalid, and get a new connection from the pool, if get connection
